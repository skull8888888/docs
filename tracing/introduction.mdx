...
LLM calls, vector DBs calls, and more. See [Configure automatic instrumentation](/tracing/configuration#configure-automatic-instrumentation)
to learn more about granular configuration or opting out.

You can trace other parts of your code as well by adding `observe`.
...
const validateTopic = async (topic: string) => {
  // assume you call the database to validate if the topic exists
  const result = await callDb(topic);
  if (result) {
    return result;
  } else {
    return "Topic not found";
