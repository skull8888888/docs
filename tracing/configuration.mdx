...
instrumentation afterwards is not possible. </Tip>
...
Let's say we call OpenAI and Antrhopic models to perform the same task, and 
...
explanation.""" + f"User message: {user_message}"
    ...
    if user_intent.choices[0].message.content[:3].lower() == "yes":
        # likely some RAG here to enrich the content
        # ...
