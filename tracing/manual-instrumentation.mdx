...
We still recommend using `L.set_span_output` and `L.set_span_attributes` to set attributes, unless you want
...
with L.start_as_current_span(name="handler", input=user_message):
    # your code here
    L.set_span_attributes({"some_attribute": "some_value"})
    L.set_span_output(result) # this will set the output of the current span
...
span = None
try:
    span = L.start_as_current_span(name="handler", input=user_message)
    # your code here
    L.set_span_output(result) # this will set the output of the current span
finally:
    if span is not None:
...
Use `with L.start_as_current_span` to create a span and set its attributes.

```python
from lmnr import Laminar as L, Attributes
...
```javascript
import { Laminar as L, LaminarAttributes } from '@lmnr-ai/lmnr';
