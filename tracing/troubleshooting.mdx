...
This typically happens when an exception occurs before the parent span is properly ended. It could also be that everything is sent correctly,
and you are just looking at the trace too early, before the parent span is sent.
...
const { Laminar: L } = require('@lmnr-ai/lmnr');
L.initialize({ projectApiKey: "...", });
```
...
For Next JS in particular, you can also disable most of the noise by setting `NEXT_OTEL_FETCH_DISABLED=1`. [Read more](/tracing/nextjs#2-initialize-laminar).
