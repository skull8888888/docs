...
This often happens in Edge runtimes, or in one-off scripts that are not running in a long-lived process.
...
Use Laminars `shutdown()` function to ensure that the traces are sent before the process exits.
...
This typically happens when an exception occurs before the parent span is properly closed.

Common causes:
- `observe()` function is not awaited in JS
- span is [created manually](/tracing/introduction#manually-creating-spans) in Python, but 
`with` statement is not used, or the span is not ended properly in the `finally` block.

If this happens elsewhere, especially in Python's `@observe` decorator, it is likely a bug in the SDK.
Please report it in our [Discord server](https://discord.gg/nNFUUDAKub).
...
`@observe` decorator in Python is not capturing the spans when I use async generators, e.g. when I am streaming responses from a model.
