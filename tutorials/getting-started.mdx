...
A Laminar project is a collection of [pipelines](/pipeline/introduction),
[endpoints](/endpoints/introduction),
[datasets](/datasets/introduction),
[evaluations](/evaluations/introduction),
and execution [logs](/logs/introduction).
...
On pro and start-up tiers, you can create more workspaces and add people to them. This enables collaboration
including real-time editing of the pipelines. To do that, you only need to know the email address with which they have registered to Laminar.
...
and navigating in the builder page, see the [pipelines documentation](/pipeline/introduction).

### Endpoints

Think of every pipeline that you build as a large function with input parameters and one or many outputs.
Laminar allows you to deploy every pipeline to endpoints and then call it externally from your code.

Every input node of the pipeline will serve as an input parameter in the API call and the outputs will
be returned in the response. See [endpoints documentation](/endpoints/introduction) for more.
