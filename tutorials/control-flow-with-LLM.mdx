...
<Warning>
This tutorial is outdated. Please refer to the [Pipeline builder](/pipeline/introduction) for the latest instructions.
</Warning>
...
Laminar.initialize(project_api_key=os.environ['LAMINAR_API_KEY'])
...
const result = await Laminar.run({
  pipeline: 'LLM-or-WebSearch',
  inputs: {
    "chat_messages": [
      {
