...
When you run a pipeline with semantic router node, it embeds all examples in each route, and then clusters them together and store the "coordinates" of the "centroid" of each cluster. The input of the pipeline is also be embedded. Then, semantic distance between the input's embedding and the centorid embeddings is calculated. The closest centroid is selected, and the output is directed to that route.
...
The pipeline in the screenshot below will only keep the names of desserts for a simple recipe generator
