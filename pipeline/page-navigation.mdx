...
### Changing prompts / node settings

Click on a node and then on a gear icon above it to edit it. Here you can change the node's name, work on the prompts,
and set the node's settings. For a better idea of what you can configure on an LLM node,
[Read more](/nodes/LLM).
...
- "Commit" button to [create a new commit](/pipeline/run-save-deploy#commit-a-new-version).
- "Set as Target" button to [set a commit version as target](/pipeline/run-save-deploy#deploying-a-pipeline-version).
...
In addition, here you can control quick access to the [env variables](/pipeline/env-variables) required for execution.
...
Fill the inputs in and click run or press Ctrl/Cmd + Enter.
