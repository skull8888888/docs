...
such as [Instruct node](/nodes/chat-instruct-llm-nodes#instruct-node),
[Chat node](/nodes/chat-instruct-llm-nodes#chat-node),
...
You can use Handles to connect nodes, which are small holes on the left or right side of each node.
...
For relevant nodes, i.e. [Chat](/nodes/chat-instruct-llm-nodes#chat-node) and [Instruct](/nodes/chat-instruct-llm-nodes#instruct-node),
we allow the system message to be templated with the user input for each call.
This is achieved with enclosing your `{{variables}}` in double curly braces, a.k.a moustache syntax.
To start, simply enclose a word in your node's instruction/system message field, and a new handle of type `String`,
and named with that enclosed word will appear on the left hand side of the node.

<Frame caption="Including {{topic}} dynamic input in instruction results in creation of 'topic' input handle">
<img
    height="300"
    src="/images/pipeline-builder/string-template-simple.png" alt="instruct node with the word 'topic' enclosed in it double curly braces"
    />
</Frame>
