...
- Executor takes in a `Map<string, any>` (replaced with `data`) parameter and returns anything
- Evaluator takes in a `Map<string, any` (replaced with `target`) parameter, and the output of the executor
- Evaluator returns either a single numeric score or a JSON object, with string keys and numeric values for multiple scores
...
Read our evals [quickstart guide](/tutorials/javascript-offline-eval) to get started with evaluating your code.
