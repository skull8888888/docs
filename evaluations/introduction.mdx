...
- Datapoint – two fixed JSON objects: `target` and `data` each with any keys.
...
Create a file named `my_first_evaluation.py` and add the following code:
...
lmnr eval my_first_evaluation.py
```
</Tab>
<Tab title = "JavaScript/TypeScript">
```sh
