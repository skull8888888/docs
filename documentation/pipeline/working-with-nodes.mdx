...
such as [LLM node](/documentation/nodes/LLM),
[Semantic Search node](/documentation/nodes/semantic-search),
[Semantic Switch node](/documentation/nodes/semantic-switch), etc.
...
This is useful in conditional competing flows and [loops](/documentation/pipeline/working-with-nodes#loops-and-cyclic-flows)
...
For some nodes, including [Semantic Switch node](/documentation/nodes/semantic-switch), the result of the node execution (and hence the entire pipeline flow) will be directed to just one of the outputs.
The selection depends on the node configuration and on the input data it receives.

## Dynamic inputs

For relevant nodes, e.g. [LLM node](/documentation/nodes/LLM),
...
Read more about variable requirements in: [String template node](/documentation/nodes/string-template) and [JSON extractor node](/documentation/nodes/json-extractor)
