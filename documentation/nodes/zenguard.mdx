...
It has [conditional output handles](/documentation/pipeline/working-with-nodes#conditional-output-handles); that is, it will redirect the flow to only one of its outputs.
...
The node directs the flow to the "block" branch if any of the detectors — which are applied one after another — detect undesired behavior; if there is nothing "detected," then it will direct the flow to "passthrough" [(Read more)](/documentation/nodes/zenguard/#passthrough-or-block).
...
This node requires `ZENGUARD_API_KEY` to be set in the ["env variables"](/documentation/pipeline/env-variables) page.
