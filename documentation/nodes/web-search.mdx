...
Another alternative is to only use relevant chunks of text from each scraped page by enabling [Only semantically similar chunks](/documentation/nodes/web-search#only-semantically-similar-chunks).

Currently, this node uses Google search for searching, so you'll need to provide your google search keys [(Read more)](/documentation/nodes/web-search#env-variables). No keys need to be provided for web scraping, since we manage it by ourselves.
...
- `content` - The whole content of the scraped page OR chunk's content, depending on if [Only semantically similar chunks](/documentation/nodes/web-search#only-semantically-similar-chunks) is enabled. Note that when we scrape the web page's content, we remove the html boilerplate and try to convert to markdown.
...
You'll need to set the following variables at [env variables](/documentation/pipeline/env-variables) page:
