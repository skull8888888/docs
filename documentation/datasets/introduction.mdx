...
1. Provide inputs and expected outputs for [Evaluations](documentation/evaluations/introduction).
...
- `target` – data additionally sent to the evaluator pipeline. See [evaluations](/documentation/evaluations/introduction)

For every key inside `data` and `target`, the value can either be a string or
a list of ChatMessage objects (see [Handle Types](/documentation/pipeline/working-with-nodes#handle-types)).
...
Datasets can be used as a [Semantic Search node](documentation/nodes/semantic-search)'s datasource.
...
See more in the [Evaluations](documentation/evaluations/introduction) page.